 ...
 * Does not overwrite existing rows (admin edits in the DB are preserved).
 *
 * Natural keys:
 *   topic     -> slug
 *   category  -> (topic_id, title)
 ...
 * user-facing app never references the sources.
 */
...
const DB_PATH = process.env.DB_PATH || "/data/life-app.db";
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            how: "Perform after every climbing session: push-ups 3x15, reverse flys with a band 3x12, external shoulder rotations with a band 3x15 each arm, reverse wrist curls 3x20. Skipping these on high-volume sessions — when you are most tired — is when the imbalance causes the most damage.",
        ...
        items: [
          {
            ...
            how: "On your easy runs, try saying a full sentence out loud every few minutes. If you cannot complete it without pausing for breath, slow down. If this feels embarrassingly slow, that means your usual pace is not as easy as you thought.",
          },
          {
            ...
            how: "Think of the trunk as a rigid column from hips to shoulders. Every 10 minutes, do a posture check: is your chest open? Is your head over your hips? If you notice yourself collapsing, shorten the stride briefly to reset your trunk position before continuing.",
          },
          {
        ...
        items: [
          {
            ...
            why: "Achilles tendinopathy is the most common injury that ends running comebacks. Eccentric calf loading is the most evidence-supported prevention method. Doing it permanently, even when symptom-free, maintains tendon resilience as training load increases.",
            how: "Stand on a step with heels off the edge. Rise on both feet. Transfer weight to one foot. Lower slowly over 3 seconds. Use both straight-leg (gastrocnemius) and bent-knee (soleus) variations in each session. Add load when 3x12 feels easy.",
    ...
    categories: [
      {
        ...
        items: [
          ...
          },
          {
            ...
            why: "Every action has an activation energy. Small amounts of friction reliably prevent habits from forming. Making a habit easy does not mean making it effortless; it means removing the obstacles between intention and the first step.",
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            how: "Walk through your environment and audit: what does it make easy? What does it make hard? Redesign it. Put fruit on the counter, not the shelf. Charge the phone in the kitchen, not the bedroom.",
          },
          {
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            why: "Having a minimum version prevents an all-or-nothing pattern. A minimum viable habit keeps the identity intact on bad days and tends to lead to more once started.",
          ...
          },
          {
            ...
            how: "Focus on consistency over intensity. One push-up every day for 3 months builds more identity evidence than 100 push-ups twice in one week. The question before every session: 'Can I do the minimum to maintain the identity today?'",
          },
        ],
      },
      {
        ...
        items: [
          {
            ...
            how: "Sit relaxed. Take 3 normal nose breaths. After the third exhale, pinch your nose. Time how long until the first real urge to breathe — the first diaphragm twitch or swallow. Stop at that first urge, not the maximum hold. Targets: below 10 seconds needs work, 10-20 is average, 20-40 is well-trained, 40+ is elite. Measure every Monday morning before coffee.",
          },
          {
        ...
        items: [
          {
            ...
            how: "As soon as the point ends, take one slow breath: inhale through the nose for 4 seconds, exhale through the mouth for 6 seconds. The extended exhale drives the parasympathetic response. Do this before your pre-point ritual, as the first act of the transition.",
          },
          {
          ...
          },
          {
            ...
            how: "Place one hand on the chest and one on the belly. Breathe in through the nose. The belly hand should rise; the chest hand should remain almost still. Practise for 5 minutes daily until it becomes the resting default. Morning box breathing performed correctly trains this pattern automatically.",
          },
          {
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            why: "Mouth breathing during sleep bypasses the nose's filtering, humidifying, and nitric oxide production functions. It tends to produce snoring and lighter sleep stages. Nasal breathing during sleep is associated with better sleep quality and lower resting heart rate on waking.",
            how: "Use medical micropore tape or purpose-made mouth tape. Start by wearing it for 10 minutes while seated and awake to build comfort. Progress over 2 weeks before using it overnight. Only use when nasal passages are clear. Do not use if you have sleep apnea without consulting a doctor.",
          },
          {
            ...
            how: "Download a sleep recording app. Place the phone on your nightstand and run a recording for one full night. Review in the morning. Look for patterns: does snoring correlate with a particular sleep position? Is it present throughout or only in the early hours? Use the data to guide which intervention to try first.",
          },
          {
            ...
            how: "Follow these rules absolutely: (1) Never perform breath holds in or near water without a trained buddy present and watching. (2) Never drive immediately after an intense breath-hold session. (3) Stop immediately if you feel lightheaded, see visual disturbances, or feel abnormal tingling. Resume the next session only. (4) Always perform CO2 table sessions lying on a flat surface, never standing.",
          },
        ],
      },
    ],
...
const seed = db.transaction(() => {
  ...
  for (let ti = 0; ti < SEED.length; ti++) {
    ...
    if (existingTopic) {
      ...
      const result = insertTopic.run(topic.slug, topic.title, topic.icon, topic.description, ti);
    ...
    for (let ci = 0; ci < topic.categories.length; ci++) {
      const category = topic.categories[ci];

      let categoryId;
      const existingCategory = findCat.get(topicId, category.title);
      if (existingCategory) {
        categoryId = existingCategory.id;
      } else {
        const result = insertCat.run(topicId, category.title, ci);
      ...
      for (let ii = 0; ii < category.items.length; ii++) {
        ...
        const existingItem = findItem.get(categoryId, item.title);
        if (!existingItem) {
          insertItem.run(
            categoryId,
            item.title,
...
try {
  const { topicsAdded, categoriesAdded, itemsAdded } = seed();
  if (topicsAdded + categoriesAdded + itemsAdded === 0) {
    console.log("seed-library: nothing to add — all content already present.");
  } else {
    console.log(
      `seed-library: done. Added ${topicsAdded} topic(s), ${categoriesAdded} category/ies, ${itemsAdded} item(s).`
    );
  }
} catch (err) {
