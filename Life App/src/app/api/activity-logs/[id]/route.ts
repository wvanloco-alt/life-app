...
export async function PATCH(
  ...
  if (body.activityTypeId !== undefined)
    updates.activityTypeId = body.activityTypeId;
  ...
  if (body.metrics !== undefined)
    updates.metrics = JSON.stringify(body.metrics);
...
export async function DELETE(
  ...
  const { id } = await params;
  await db.delete(activityLogs).where(eq(activityLogs.id, parseInt(id)));
