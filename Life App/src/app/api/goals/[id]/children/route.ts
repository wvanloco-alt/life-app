...
export async function GET(
  ...
  if (isNaN(parentId)) return NextResponse.json({ error: "Invalid goal ID" }, { status: 400 });

  const parent = await db.select({ id: goals.id }).from(goals).where(and(eq(goals.id, parentId), eq(goals.userId, userId)));
  if (parent.length === 0) return NextResponse.json({ error: "Goal not found" }, { status: 404 });

  const children = await db.select().from(goals).where(and(eq(goals.parentGoalId, parentId), eq(goals.userId, userId)));
  ...
  const result = await Promise.all(
    children.map(async (child) => {
      const childRoles = allGR.filter((r) => r.goalId === child.id).map((r) => ({ id: r.roleId, name: r.roleName, color: r.roleColor }));
      ...
      if (child.month) {
        ...
        const tallies = await db.select({ count: goalTallies.count }).from(goalTallies).where(and(eq(goalTallies.goalId, child.id), gte(goalTallies.date, from), lte(goalTallies.date, to)));
        current += tallies.reduce((s, t) => s + t.count, 0);
      }

      const percentage = target > 0 ? Math.min(100, Math.round((current / target) * 100)) : 0;
      return { ...child, quadrant: deriveQuadrant(child.targetDate), roles: childRoles, progress: { current, target, percentage } };
    })
  );
