...
async function getOrCreateBudgetSettings() {
  ...
  const [created] = await db
    ...
    .values({ currency: "EUR", monthlySavingsTarget: 0 })
    .returning();
  return created!;
}
...
export async function GET(request: NextRequest) {
  ...
  const monthEnd = format(
    endOfMonth(parseISO(monthStart)),
    "yyyy-MM-dd"
  );
  ...
  const totalFixedCosts = allFixedCosts
    .filter(
      (fc) =>
        fc.endMonth == null || (fc.startMonth <= month && fc.endMonth >= month)
    )
    .reduce((sum, fc) => sum + fc.amount, 0);

  const monthlySavingsTarget = settings.monthlySavingsTarget ?? 0;
  const spendingBudget =
    totalIncome - totalFixedCosts - monthlySavingsTarget;

  const allSpending = await db.select().from(spendingEntries);
  const monthSpending = allSpending.filter(
  ...
  const dailyAllowance =
    daysLeft > 0 ? Math.round((remaining / daysLeft) * 100) / 100 : 0;

  const byCategory = new Map<
    string,
    { amount: number; icon: string; color: string }
  >();

  const activeFixedCosts = allFixedCosts.filter(
    (fc) =>
      fc.endMonth == null || (fc.startMonth <= month && fc.endMonth >= month)
  );
  for (const fc of activeFixedCosts) {
    const cur = byCategory.get(fc.category) ?? {
      amount: 0,
      icon: "🏠",
      color: "#6B7280",
    };
  ...
  const categories = await db
    ...
    .where(eq(spendingCategories.isArchived, false));
  ...
  const monthPlannedExpenses = await db
    ...
    .where(eq(plannedExpenses.month, month));

  const totalPlannedExpenses = monthPlannedExpenses.reduce(
    (sum, pe) => sum + pe.amount,
    0
  );

  const catMap = new Map(categories.map((c) => [c.name, c]));
  const spendingByCategory = Array.from(byCategory.entries()).map(
    ([category, { amount }]) => {
  ...
  if (settings.savingsGoalTotal != null && settings.savingsGoalTotal > 0) {
    ...
    const allSpendingAll = await db.select().from(spendingEntries);
    const allFixed = await db
      .select()
      .from(fixedCosts)
      .where(eq(fixedCosts.isActive, true));
    const allPlanned = await db.select().from(plannedExpenses);
    ...
    while (d <= currentMonthDate) {
      ...
      const recurringBySource = new Map<string, number>();
      for (const e of allIncome) {
        if (e.isRecurring) recurringBySource.set(e.source, e.amount);
      }
      let mRecurring = 0;
      const mSources = new Set(
        allIncome.filter((e) => e.month === m).map((e) => e.source)
      );
      for (const [source, amt] of recurringBySource) {
        if (!mSources.has(source)) mRecurring += amt;
      }

      const mFixed = allFixed
        .filter(
          (fc) =>
            fc.endMonth == null || (fc.startMonth <= m && fc.endMonth >= m)
        )
        .reduce((s, fc) => s + fc.amount, 0);

      const mSpending = allSpendingAll
        .filter((e) => e.date >= mStart && e.date <= mEnd)
        .reduce((s, e) => s + e.amount, 0);

      const mPlanned = allPlanned
        .filter((pe) => pe.month === m)
    ...
    const percentage =
      total > 0
        ? Math.min(100, Math.round((saved / total) * 10000) / 100)
        : 0;

    savingsGoal = {
      total,
      targetDate: settings.savingsGoalTargetDate,
      saved,
      percentage,
    };
  }
  ...
  const summary: BudgetSummary = {
    ...
    plannedExpenses: monthPlannedExpenses,
  };
