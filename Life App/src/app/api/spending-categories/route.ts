...
async function seedDefaultCategories() {
  const existing = await db.select({ id: spendingCategories.id }).from(spendingCategories).limit(1);
...
export async function GET(request: NextRequest) {
  ...
  const result = await db
    ...
    .where(includeArchived ? undefined : eq(spendingCategories.isArchived, false))
...
export async function POST(request: NextRequest) {
  ...
  const maxOrder =
    (
      await db
        .select({ displayOrder: spendingCategories.displayOrder })
        .from(spendingCategories)
    ).reduce((max, r) => Math.max(max, r.displayOrder), -1) + 1;
