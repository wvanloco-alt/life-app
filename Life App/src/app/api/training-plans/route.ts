...
export async function GET(request: NextRequest) {
  ...
  const plans = await db
    .select()
    .from(trainingPlans)
    .where(eq(trainingPlans.goalId, parseInt(goalId)));
  ...
  const phases = await db
    .select()
    .from(trainingPhases)
    .where(eq(trainingPhases.trainingPlanId, plan.id));
...
export async function POST(request: NextRequest) {
  ...
  if (!goalId || yearsExperience == null || !startDate) {
    return NextResponse.json(
      { error: "goalId, yearsExperience, and startDate are required" },
      { status: 400 }
    );
  }
  ...
  if (existing.length > 0) {
    return NextResponse.json(
      { error: "This goal already has a training plan" },
      { status: 409 }
    );
  }
  ...
  if (sport === "running") {
    ...
    if (profile?.goalDistance == null || profile?.runsPerWeek == null) {
      return NextResponse.json(
        { error: "Running plans require goalDistance and runsPerWeek in sportProfile" },
        { status: 400 }
      );
    }
    const assessment = assessRunningLevel(
      profile.runsPerWeek,
      yearsExperience,
      profile.canRun30MinContinuous ?? false,
      profile.hasRaced ?? false
    );
    derivedLevel = assessment.derivedLevel;
    recommendedModel = assessment.recommendedModel;
    generatedPhases = generateRunningPhases(
      assessment.derivedLevel as RunnerLevel,
      profile.goalDistance,
      (profile.physicalLimitations ?? []) as RunningLimitation[],
      startDate
    );
  } else if (sport === "tennis") {
    const profile = sportProfile as TennisSportProfile;
    if (!profile?.selfRating || !profile?.playingStyle) {
      return NextResponse.json(
        { error: "Tennis plans require selfRating and playingStyle in sportProfile" },
        { status: 400 }
      );
    }
    ...
    derivedLevel = assessment.derivedLevel;
    recommendedModel = assessment.recommendedModel;
    generatedPhases = generateTennisPhases(
      assessment.derivedLevel as TennisPlayerLevel,
      profile.playingStyle as TennisPlayingStyle,
      (profile.physicalLimitations ?? []) as PhysicalLimitation[],
      startDate
    );
  } else {
    const profile = sportProfile as ClimbingSportProfile;
    if (!profile?.maxBoulderGrade || !profile?.maxSportGrade || !profile?.discipline) {
      return NextResponse.json(
        { error: "Climbing plans require discipline, maxBoulderGrade, and maxSportGrade in sportProfile" },
        { status: 400 }
      );
    }
    const assessment = assessLevel(
      profile.maxBoulderGrade,
      profile.maxSportGrade,
      yearsExperience
    );
    derivedLevel = assessment.derivedLevel;
    recommendedModel = assessment.recommendedModel;
    generatedPhases = generatePhases(
      derivedLevel as any,
      profile.discipline,
      (profile.physicalLimitations ?? []) as ClimbingLimitation[],
      startDate
    );
  }
  ...
  return NextResponse.json(
    { ...plan, sportProfile: parseSportProfile(plan.sportProfile), phases: insertedPhases },
    { status: 201 }
  );
}
