...
async function seedDefaultActivityTypes() {
  const existing = await db.select({ id: activityTypes.id }).from(activityTypes).limit(1);
...
export async function GET() {
  ...
  const result = await db.select().from(activityTypes);

  const parsed = result.map((s) => {
    let metricsConfig = [];
    let variants = null;
    try { metricsConfig = JSON.parse(s.metricsConfig); } catch { /* invalid JSON, use default */ }
...
export async function POST(request: NextRequest) {
  ...
  return NextResponse.json(
    {
      ...created,
      metricsConfig: JSON.parse(created.metricsConfig),
      variants: created.variants ? JSON.parse(created.variants) : null,
    },
    { status: 201 }
  );
}
