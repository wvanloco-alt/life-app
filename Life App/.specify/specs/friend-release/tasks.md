...
- [ ] T004 Create `src/lib/auth.ts` — NextAuth config with Credentials provider. Validates username/password against `users` table using bcryptjs. Session includes `user.id` and `user.role`.
- [ ] T005 Create `src/app/api/auth/[...nextauth]/route.ts` — NextAuth route handler (GET and POST exports).
- [ ] T006 Create `src/app/login/page.tsx` — simple login form (username + password inputs, submit button). Calls `signIn("credentials", ...)` from NextAuth. Shows error on failure.
- [ ] T007 Create `src/middleware.ts` — protects all routes except `/login` and `/api/auth/*`. Redirects unauthenticated requests to `/login`.
- [ ] T008 Add `SessionProvider` wrapper to `src/app/layout.tsx` so session is available app-wide.
- [ ] T009 Add a "Log out" button to `src/components/layout/app-sidebar.tsx` that calls `signOut()`.
- [ ] T010 Create a seed script `src/scripts/create-admin.ts` that creates the first admin user (you) with a hashed password. Run once: `npx tsx src/scripts/create-admin.ts`.
...
**⚠️ Do not start Phase 3 until the migration runs without errors.**
...
- [ ] T012 Generate and run migration: `npm run db:generate` then `npm run db:migrate`. Existing rows will get `user_id = ""` (the default). We will backfill in T013.
- [ ] T013 Create a one-time backfill script `src/scripts/backfill-user-id.ts` that sets `user_id = <your admin user ID>` on all existing rows in all tables from T011. Run it once after migration: `npx tsx src/scripts/backfill-user-id.ts`.
...
- [ ] T014 [P] Update `src/app/api/roles/route.ts` — scope GET and POST by `userId`
- [ ] T015 [P] Update `src/app/api/roles/[id]/route.ts` — scope PATCH and DELETE by `userId`
- [ ] T016 [P] Update `src/app/api/goals/route.ts` — scope GET and POST by `userId`
- [ ] T017 [P] Update `src/app/api/goals/[id]/route.ts` — scope GET, PATCH, DELETE by `userId`
- [ ] T018 [P] Update `src/app/api/goals/[id]/progress/route.ts` — scope by `userId`
- [ ] T019 [P] Update `src/app/api/goals/[id]/session-patterns/route.ts` — scope by `userId`
- [ ] T020 [P] Update `src/app/api/goal-tallies/route.ts` — scope by `userId`
- [ ] T021 [P] Update `src/app/api/goal-tallies/[id]/route.ts` — scope by `userId`
- [ ] T022 [P] Update `src/app/api/activities/route.ts` — scope GET and POST by `userId`
- [ ] T023 [P] Update `src/app/api/activities/[id]/route.ts` — scope PATCH and DELETE by `userId`
- [ ] T024 [P] Update `src/app/api/activities/summary/route.ts` — scope by `userId`
- [ ] T025 [P] Update `src/app/api/recurring-activities/route.ts` — scope by `userId`
- [ ] T026 [P] Update `src/app/api/recurring-activities/[id]/route.ts` — scope by `userId`
- [ ] T027 [P] Update `src/app/api/scheduler-settings/route.ts` — scope by `userId`
- [ ] T028 [P] Update `src/app/api/scheduler-blackout-dates/route.ts` — scope by `userId`
- [ ] T029 [P] Update `src/app/api/scheduler-blackout-dates/[id]/route.ts` — scope by `userId`
- [ ] T030 [P] Update `src/app/api/schedule/generate/route.ts` — scope by `userId`
- [ ] T031 [P] Update `src/app/api/schedule/apply/route.ts` — scope by `userId`
- [ ] T032 [P] Update `src/app/api/schedule/reset/route.ts` — scope by `userId`
- [ ] T033 [P] Update `src/app/api/activity-types/route.ts` — scope by `userId`
- [ ] T034 [P] Update `src/app/api/activity-types/[id]/route.ts` — scope by `userId`
- [ ] T035 [P] Update `src/app/api/activity-logs/route.ts` — scope by `userId`
- [ ] T036 [P] Update `src/app/api/activity-logs/[id]/route.ts` — scope by `userId`
- [ ] T037 [P] Update `src/app/api/body-metrics/route.ts` — scope by `userId`
- [ ] T038 [P] Update `src/app/api/body-metrics/[id]/route.ts` — scope by `userId`
- [ ] T039 [P] Update `src/app/api/budget-settings/route.ts` — scope by `userId`
- [ ] T040 [P] Update `src/app/api/budget/summary/route.ts` — scope by `userId`
- [ ] T041 [P] Update `src/app/api/spending/route.ts` — scope by `userId`
- [ ] T042 [P] Update `src/app/api/spending/[id]/route.ts` — scope by `userId`
- [ ] T043 [P] Update `src/app/api/spending-categories/route.ts` — scope by `userId`
- [ ] T044 [P] Update `src/app/api/spending-categories/[id]/route.ts` — scope by `userId`
- [ ] T045 [P] Update `src/app/api/fixed-costs/route.ts` — scope by `userId`
- [ ] T046 [P] Update `src/app/api/fixed-costs/[id]/route.ts` — scope by `userId`
- [ ] T047 [P] Update `src/app/api/income/route.ts` — scope by `userId`
- [ ] T048 [P] Update `src/app/api/income/[id]/route.ts` — scope by `userId`
- [ ] T049 [P] Update `src/app/api/planned-expenses/route.ts` — scope by `userId`
- [ ] T050 [P] Update `src/app/api/planned-expenses/[id]/route.ts` — scope by `userId`
- [ ] T051 [P] Update `src/app/api/training-plans/route.ts` — scope by `userId`
- [ ] T052 [P] Update `src/app/api/training-plans/[id]/route.ts` — scope by `userId`
...
- [ ] T060 Create `src/lib/seed-user-defaults.ts` — a function `seedUserDefaults(userId: string)` that:
...
- [ ] T061 Call `seedUserDefaults(userId)` inside the NextAuth `signIn` callback in `src/lib/auth.ts` — runs on every login but is idempotent so safe to repeat.
- [ ] T062 Remove or guard any existing global seeding logic that runs without a `userId` (check `src/app/api/*/route.ts` files that seed defaults on first GET — they must now check by userId).
...
- [ ] T063 Create `src/app/admin/users/page.tsx` — admin-only page showing all users (username, role, active status, created date). Includes a "Create User" form and a "Deactivate" toggle per user.
- [ ] T064 Create `src/app/api/admin/users/route.ts` — GET (list users) and POST (create user with hashed password). Checks `session.user.role === "admin"` — returns 403 if not.
- [ ] T065 Create `src/app/api/admin/users/[id]/route.ts` — PATCH to toggle `isActive`. Admin-only. Prevent deactivating the last admin account.
- [ ] T066 Add an "Admin" link to `src/components/layout/app-sidebar.tsx` that only renders when `session.user.role === "admin"`.
...
**Purpose**: Deploy the app to Railway (or Fly.io) so friends can access it via a real URL.

- [ ] T067 Add required environment variables to `.env.example`:
...
- [ ] T072 Deploy and verify: visit the public URL → see login page → log in → full app works → log out.
- [ ] T073 Restart the Railway container and confirm data is not lost (SQLite volume persistence test).

**Checkpoint**: Share the URL with one friend. They log in with their account. They use the app. You restart the deployment. Their data is still there. ✓
...
- [ ] T078 Update `specs/master/data-model.md` to document the `users` table and the `user_id` column on all tables.
...
| Phase 7 (Polish) | Phase 6 complete | Yes — T074–T078 in parallel |
