...
5. **Given** an API client (or a future buggy form) POSTs a goal payload with `sessionsPerWeek = 100`, **When** the server stores the goal, **Then** the persisted row has `sessionsPerWeek = 7` (server-side clamp per FR-016), not 100; the scheduler then proposes at most 7 sessions for that goal in any ISO week (subject to the existing daily and per-day caps).
...
- **FR-010**: The role-form UI MUST remove the "Scheduling Rules" section in full. Two files are affected:
  - `src/components/roles/role-form.tsx`: remove the two number inputs, their labels, their helper text, the section heading, and the form state hooks (`useState`) that back them. The work-role toggle MUST remain.
  - `src/components/roles/role-list.tsx`: in the local `handleSave` data parameter type (around lines 46-53 at the time of writing), remove the `maxWeeklyOccurrences: number` and `minRestDays: number` fields so it matches the new shape passed by the form. No other change in this file.
...
- **FR-012**: The scheduler MUST remove the `roleDaySessions` map: its construction in `generateSchedule` (lines ~478-492 at the time of writing), its parameter on every helper that currently receives it (four helpers plus `violatesRestConstraints` itself, five function signatures total), and the write-back in `commitSession` (lines ~981-984). This is a self-contained cleanup: nothing else reads the map (verified during scope phase).
...
- **FR-016**: `POST /api/goals` and `PATCH /api/goals/[id]` MUST clamp `sessionsPerWeek` to the inclusive integer range `[1, 7]` before persisting. POST: a submitted value above 7 is stored as 7; a submitted value below 1 (including 0, negative numbers, and floats that round below 1) is stored as 1; an absent or non-finite (`NaN`, `Infinity`) field falls back to the existing default of 3. PATCH: a `sessionsPerWeek` field that is absent or `undefined` means "no change" (the prior value is preserved); a `sessionsPerWeek` field that is explicitly provided is clamped using the same `[1, 7]` rule as POST. Rationale: with role-level caps gone, `goal.sessionsPerWeek` plus `enforceWeeklySpread` is the only goal-level weekly cap. The client form enforces this via `min={1} max={7}`, but a direct API call or future buggy client must not be able to write an out-of-range value. The clamp closes the API-direct-write gap surfaced by the spec review.
...
- **SC-009**: A POST to `/api/goals` with `sessionsPerWeek = 100` results in the persisted row carrying `sessionsPerWeek = 7`, not 100. Verified by a new unit or integration test against the goals route handler.
