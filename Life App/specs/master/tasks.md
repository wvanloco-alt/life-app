...
> Last updated: 2026-05-16. Tracks completed work across all features.
...
| 2026-05-15 | Small UX fixes batch (PRs #11–#15) | (1) Activity-type-filtered goal picker: goal dropdown in WorkoutLog and Schedule Activity now only shows goals whose `activityTypeId` matches the selected activity type; resets to null when type changes. (2) Activity type edit form: fixed stale/empty form bug by adding `key={editingType?.id}` to force React remount on each distinct type. (3) Goal-progress sessions fix (hotfix PR #15): yearly and monthly goals with `targetMetric: null` were not counting `activity_logs`; fixed the progress query to include logs regardless of metric. (4) Goals delete: removed the non-functional "Delete" option from the goals kebab menu; archive is the correct action. (5) Body-metric sparkline RangeError: guarded the sparkline calculation against empty arrays to prevent crashes on the Activities dashboard. |
| 2026-05-15 | Habits UI — empty state redesign (PR #22) | Removed redundant "Habits" h1 and top-right button from the empty state. New `HabitEmptyState` owns the full page: centered title, subtitle, primary "Walk me through it" CTA, secondary "Add a habit" text link, and editorial principles below. |
| 2026-05-16 | Habits UI — 3-week HabitCalendar + full-width rows (PRs #23–#25) | New `HabitCalendar` component: shows last/this/next calendar week (Mon–Sun) with week labels, day labels, 44px cells, today ring, future dates muted. HabitRow redesigned as a full-width row (no card border): identity block (Fraunces, w-60) on the left, 3-week calendar filling remaining width on the right. Rows separated by thin dividers. HabitPrinciples gains `horizontal` prop (grid-cols-3 below the list, full width). `HabitCalendar` extracted from `HabitStrip`. Always-visible pencil icon replaces hidden kebab edit. |
| 2026-05-16 | Habits UI — page padding and typography polish (PR #26) | Added `px-6 py-8` outer padding to HabitList (matches Goals page benchmark). Identity text bumped to 17px; name subtitle to text-sm (14px); streak, day labels, week labels all increased. Left column widened to w-60; row gap to gap-12; row padding to py-8. Icons from w-3.5 to w-4. Skeleton updated to match. |
| 2026-05-16 | Goal archive cascade + user reset script (PR #27) | Bug fix: `PATCH /api/goals/:id` now cascades `status` to monthly children when value is `"archived"` or `"active"` (restore). Completing a yearly goal does NOT cascade. New `scripts/reset-user-data.cjs` + `scripts/reset-user-data-lib.cjs`: Node.js utility (uses better-sqlite3) that deletes all data for a given username in FK-safe order while keeping the user account intact. Integrated into `apply-schema.js` via `reset-user-data-lib.cjs`. |
| 2026-05-19–22 | Library Phase 1 — foundation (PR #30) | Four new tables: `library_topics`, `library_categories`, `library_items`, `library_bookmarks` in `src/db/schema.ts` and `apply-schema.js`. TypeScript types: `LibraryTopic`, `LibraryCategory`, `LibraryItem`, `LibraryItemWithBookmark`, `LibraryCategoryWithItems`, `LibraryTopicWithCategories`, `BookmarkedItem`, `LibraryItemType`. Seed content in `scripts/seed-library-lib.cjs` (single source of truth — 5 topics, 20 categories, 91 items, no source citations per FR-002). Idempotent seed wired into `apply-schema.js` step 7. API: `GET /api/library/topics`, `GET /api/library/topics/:slug` (nested with `isBookmarked`). |
| 2026-05-22 | Library Phase 2 — navigation and topic pages (PR #31) | Sidebar Library group: five topic entries + `<Separator>` + Bookmarks link. Route `src/app/library/[slug]/page.tsx`. Components: `LibraryEmptyState`, `LibraryItemRow` (type badge + What/Why/How), `LibraryCategorySection` (sticky header), `LibraryTopicPage` (fetches API, layout-mirroring skeleton). |
| 2026-05-22 | Library Phase 3 — bookmarks (PR #32) | API: `GET/POST /api/library/bookmarks` (POST idempotent, always 201), `DELETE /api/library/bookmarks/:itemId` (always 204). Bookmark toggle wired in `LibraryItemRow` (outline → filled, optimistic). `LibraryTopicPage` manages optimistic bookmark state with revert on failure. `LibraryBookmarksPage` at `/library/bookmarks`: items grouped by topic, optimistic removal, empty state. `BookmarkedItem` type extended with `isBookmarked`, `topicId`, `topicIcon`. |
| 2026-05-22 | Library Phase 4 — admin editing and drag reorder (PR #33) | Seven admin-only API routes (all return 403 for non-admins): POST categories, PATCH/DELETE categories, POST items, PATCH/DELETE items, PUT reorder. `LibraryDeleteDialog` (confirmation dialog). `LibraryItemPanel` (right-side Sheet, full create/edit form with field-length validation). `LibraryCategorySection`: DnD sortable items (admin only), `SortableItemRow` with drag handle + edit pencil + delete. `LibraryTopicPage`: reads `session.user.role`, passes `isAdmin` down, handles all optimistic CRUD state. |
| 2026-05-22 | Library — two-column layout with sticky TOC (PR #34) | Replaced `max-w-3xl` single column with `flex` two-column layout. Left: `flex-1` content. Right: 208px sticky `LibraryToc` (hidden below `lg`). `LibraryToc` uses `IntersectionObserver` to highlight active category; clicking smooth-scrolls to section. `LibraryCategorySection` gains `id="category-{id}"` anchor. Skeleton updated to match layout. |
| 2026-08-13 | Life App 2.0 (PRs #94–#108) | Merged to `master`. Dashboard homepage with auto Garmin sync; habits year heatmap + X/30 consistency; Today's Session card; budget Forecast tab; morning email digest; settings tab refactor. New tables: `sleep_logs`, `daily_metrics`, `garmin_connections`, `email_preferences`. New column: `activity_logs.garmin_activity_id`. Deployment: Debian slim base, `gosu`, `serverExternalPackages`, `patch-garmin.cjs` postinstall. Sub-specs: `habits-and-session-card`, `budget-forecasting`, `email-morning-digest`. |
| 2026-08-14 | Garmin `tennis_v2` sync fix (PR #109) | `garmin-connect-client` v2.0.0 validates type keys in `dist/types.js`, not `index.js`. Activities now fetched via raw Garmin API in `garmin-client.ts`; postinstall patch updated to patch `types.js`. Fixes sync failures for newer tennis activity types. |
